...
export const getIngestionReport = (taskId: string): Promise<IngestionParseReport> =>
  request.get<unknown, IngestionParseReport>(`/ingestion/${taskId}/report`)

export const getIngestionEvidence = (taskId: string): Promise<EvidenceAnchor[]> =>
  request.get<unknown, EvidenceAnchor[]>(`/ingestion/${taskId}/evidence`)

export const getIngestionChunks = (taskId: string): Promise<KnowledgeChunk[]> =>
  request.get<unknown, KnowledgeChunk[]>(`/ingestion/${taskId}/chunks`)

export const getIngestionSource = (taskId: string): Promise<IngestionSourceAsset> =>
  request.get<unknown, IngestionSourceAsset>(`/ingestion/${taskId}/source`)

export const listIngestionJobs = (limit = 50): Promise<IngestionJob[]> =>
  request.get<unknown, IngestionJob[]>(`/ingestion/jobs?limit=${limit}`)
