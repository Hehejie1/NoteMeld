...
export const retryWikiExtraction = async (taskId: string): Promise<unknown> => {
  return request.post(`/wiki/retry/${encodeURIComponent(taskId)}`)
}

export const getWikiExtractionStatus = async (taskId: string): Promise<Record<string, unknown>> => {
  return request.get(`/wiki/status/${encodeURIComponent(taskId)}`)
}
