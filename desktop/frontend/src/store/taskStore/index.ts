...
export type TaskFormData = Record<string, unknown> & {
  video_url: string
...
interface TaskStore {
  ...
  linkNoteTask: (conversationId: string, noteTaskId: string, formData: TaskFormData, platform: string) => void
  ...
  retryTask: (id: string, payload?: TaskFormData, noteTaskIdOverride?: string) => Promise<void>
...
const asLegacyRecord = (value: unknown): LegacyRecord =>
  value && typeof value === 'object' && !Array.isArray(value) ? value as LegacyRecord : {}

const stringValue = (...values: unknown[]): string | undefined =>
  values.find((value): value is string => typeof value === 'string' && value.length > 0)

export const normalizeConversationMessage = (raw: LegacyRecord): ConversationMessage => {
  const createdAt = stringValue(raw.createdAt, raw.created_at) || new Date().toISOString()
  const missingMessageType = !raw.message_type
  const meta = {
    ...asLegacyRecord(raw.meta),
  ...
  return {
    ...
    role: (stringValue(raw.role) || "assistant") as ConversationMessageRole,
    message_type: (stringValue(raw.message_type) || "system_error") as ConversationMessageType,
    content: stringValue(raw.content) || (missingMessageType ? "Message missing message_type; downgraded to system_error." : ""),
    status: stringValue(raw.status) as ConversationMessageStatus | undefined,
    meta,
    createdAt,
    updatedAt: stringValue(raw.updatedAt, raw.updated_at) || createdAt,
    sources: (Array.isArray(raw.sources) ? raw.sources : []) as ConversationSource[],
    error: Boolean(raw.error) || missingMessageType,
    isStreaming: Boolean(raw.isStreaming),
  }
}

export const normalizeNoteDocument = (rawDocument: LegacyRecord): NoteDocument => ({
  taskId: stringValue(rawDocument.taskId, rawDocument.task_id) || '',
  title: stringValue(rawDocument.title) || '未命名笔记',
  content: stringValue(rawDocument.content) || '',
  sourceUrl: stringValue(rawDocument.sourceUrl, rawDocument.source_url) || '',
  platform: stringValue(rawDocument.platform) || '',
  modelName: stringValue(rawDocument.modelName, rawDocument.model_name) || '',
  style: stringValue(rawDocument.style) || '',
  status: stringValue(rawDocument.status) || '',
  wikiStatus: stringValue(rawDocument.wikiStatus, rawDocument.wiki_status) || '',
  createdAt: stringValue(rawDocument.createdAt, rawDocument.created_at) || new Date().toISOString(),
})
...
export const normalizeTask = (raw: LegacyRecord): Task => {
  const messages = (Array.isArray(raw.messages) ? raw.messages : [])
    .map(message => normalizeConversationMessage(asLegacyRecord(message)))
  const pendingIds = Array.isArray(raw.pendingNoteTaskIds)
    ? raw.pendingNoteTaskIds
    : Array.isArray(raw.pending_note_task_ids) ? raw.pending_note_task_ids : []
  const explicitPendingIds = pendingIds.filter((value): value is string => typeof value === 'string')
  return {
    id: stringValue(raw.id) || uuidv4(),
    markdown: stringValue(raw.markdown) || '',
    transcript: {
      ...defaultTranscript(),
      ...asLegacyRecord(raw.transcript),
    },
    status: (stringValue(raw.status) || 'SUCCESS') as TaskStatus,
    message: stringValue(raw.message) || '',
    audioMeta: {
      ...defaultAudioMeta(),
      ...asLegacyRecord(raw.audioMeta || raw.audio_meta),
    },
    createdAt: stringValue(raw.createdAt, raw.created_at) || new Date().toISOString(),
    platform: stringValue(raw.platform) || '',
    mode: (stringValue(raw.mode) || 'chat') as ConversationMode,
    title: stringValue(raw.title) || '',
    messages,
    linkedNoteTaskId: stringValue(raw.linkedNoteTaskId, raw.linked_note_task_id) || '',
    pendingNoteTaskIds: derivePendingNoteTaskIds(messages, explicitPendingIds),
    activeDocumentTaskId: stringValue(raw.activeDocumentTaskId, raw.active_document_task_id) || '',
    documents: (Array.isArray(raw.documents) ? raw.documents : [])
      .map(rawDocument => normalizeNoteDocument(asLegacyRecord(rawDocument))),
    noteState: (stringValue(raw.noteState, raw.note_state) || (raw.mode === 'note' ? 'generating' : 'none')) as ConversationNoteState,
    formData: {
      ...defaultFormData(),
      ...asLegacyRecord(raw.formData || raw.form_data),
    },
  }
}
...
export const useTaskStore = create<TaskStore>()((set, get) => ({
  ...
  linkNoteTask: (conversationId, noteTaskId, formData: TaskFormData, platform) =>
    set(state => ({
      tasks: state.tasks.map(task =>
  ...
  deleteNoteDocument: async (conversationId: string, taskId: string) => {
    ...
    const nextTask = normalizeTask(asLegacyRecord(await deleteConversationDocument(conversationId, taskId)))
    set(state => ({
  ...
  retryTask: async (id: string, payload?: TaskFormData, noteTaskIdOverride?: string) => {
    if (!id) {
      toast.error('任务不存在')
      return
    }
  ...
  loadConversations: async () => {
    ...
    set({
      tasks: data.map(item => normalizeTask(asLegacyRecord(item))),
  ...
  loadConversation: async (id: string) => {
    ...
    const task = normalizeTask(asLegacyRecord(data))
    set(state => ({
  ...
  retryChat: async (id: string) => {
    ...
    ;(window as Window & { __NOTEMELD_CHAT_RETRY__?: unknown }).__NOTEMELD_CHAT_RETRY__ = {
      conversationId: id,
