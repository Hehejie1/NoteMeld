...
import { ChevronRight, CircleAlert, PackageOpen, RefreshCw, Upload } from 'lucide-react'
import { Link } from 'react-router-dom'

import KnowledgeEmptyState from '@/components/KnowledgeEmptyState'
import { Button } from '@/components/ui/button'
import { installApplicationPackage, listApplications, type ApplicationSummary } from '@/services/applications'
...
const ApplicationList = () => {
  ...
  const install = async (file: File) => {
    ...
    setNotice('')
    try {
      const result = await installApplicationPackage(file)
      setNotice(`${result.manifest.name} 已提交安全审批，审批通过后请到“系统运行 → 安全审批”激活。`)
    } catch {
      setError('应用包安装失败，请检查 manifest、权限和 runtime entry')
    } finally {
      setInstalling(false)
  ...
  return (
    <div className="h-full overflow-auto bg-surface px-5 py-6 md:px-8">
      <div className="mx-auto max-w-4xl">
        <div className="flex items-start justify-between gap-4"><div><div className="text-[11px] font-medium tracking-[0.16em] text-on-surface-variant">APPLICATION HOST</div><h1 className="mt-2 font-display text-2xl font-bold text-on-surface">应用</h1><p className="mt-2 text-sm text-on-surface-variant">由 NoteMeld Host 管理的应用包及其运行状态。</p></div><div className="flex items-center gap-2"><input ref={fileInput} type="file" accept=".zip,application/zip" className="sr-only" onChange={event => { const file = event.target.files?.[0]; if (file) void install(file) }} /><Button variant="outline" size="sm" onClick={() => fileInput.current?.click()} disabled={installing}><Upload className="mr-2 h-3.5 w-3.5" />{installing ? '安装中…' : '安装应用包'}</Button><Button variant="ghost" size="sm" onClick={() => void load()} disabled={loading} aria-label="刷新应用列表"><RefreshCw className={loading ? 'animate-spin' : ''} /></Button></div></div>
        {notice ? <p role="status" className="mt-5 rounded-xl border border-primary/20 bg-primary/5 px-4 py-3 text-sm text-on-surface">{notice}</p> : null}
        {applications.length === 0 ? <div className="mt-10"><KnowledgeEmptyState title="还没有可用应用" description="应用包提交审批后，审批通过并激活才会出现在这里。" /></div> : <div className="mt-7 grid gap-3 md:grid-cols-2">{applications.map(application => <ApplicationCard key={application.id} application={application} />)}</div>}
      </div>
    </div>
  )
}
