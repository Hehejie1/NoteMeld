...
export const createDemoRuntime = (): DemoRuntime => {
  ...
  const request = async <T>({ method, path, body }: DemoRequest): Promise<T> => {
    ...
    const settings = state.settings as Record<string, unknown> & {
      ...
      models: Array<{ provider_id: string }>
      transcriber: Record<string, unknown>
      transcriberModels: unknown
      downloaderConfigured: Record<string, boolean>
      deployStatus: { mcp: unknown } & Record<string, unknown>
      usageOverview: unknown
      usageRecords: unknown
      mcpServers: Record<string, unknown>
      researchSearch: Record<string, unknown>
    }
    if (method === 'GET' && pathname === '/get_all_providers') return clone(settings.providers) as T
    if (method === 'GET' && pathname.startsWith('/get_provider_by_id/')) return clone(settings.providers.find(item => item.id === pathname.split('/').pop())) as T
    if (method === 'GET' && pathname === '/model_list') return clone(settings.models) as T
    if (method === 'GET' && pathname.startsWith('/model_enable/')) return clone(settings.models.filter(item => item.provider_id === pathname.split('/').pop())) as T
