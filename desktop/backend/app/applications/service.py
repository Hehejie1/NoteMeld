...
from app.applications.manifest import default_application_package_root, runtime_kind_for_platform, validate_manifest, validate_package
...
from app.utils.storage_paths import application_workspaces_root, data_root, note_output_dir
...
class ApplicationService:
    ...
    def install_package(self, content: bytes, *, replace: bool = False) -> dict[str, Any]:
        """Safely install an application package and refresh the trusted catalog."""
        try:
            manifest, digest = validate_package(content)
        except ValueError as exc:
            raise ApplicationError(getattr(exc, "code", "invalid_package"), str(exc), 400) from exc
        target = (self.registry.package_root / manifest["id"]).resolve()
        root = self.registry.package_root.resolve()
        target.relative_to(root)
        if target.exists() and not replace:
            raise ApplicationError("application_exists", "application is already installed; use replace to upgrade", 409)
        root.mkdir(parents=True, exist_ok=True)
        staging = Path(tempfile.mkdtemp(prefix=f".{manifest['id']}.staging-", dir=str(root)))
        ...
        try:
            ...
            command_program = ((manifest.get("runtime") or {}).get("command") or {}).get("program")
            if command_program:
                # A process-jsonl package must be runnable after extraction even
                # when the archive was produced on a filesystem that dropped
                # POSIX executable bits.
                os.chmod(staging / command_program, 0o700)
            if target.exists():
                backup = root / f".{manifest['id']}.previous-{uuid.uuid4().hex}"
                os.replace(target, backup)
            os.replace(staging, target)
            self.registry.manifests = self.registry._discover()
            self.sync_registry()
            result = self.get(manifest["id"])
            result["package_sha256"] = digest
            return result
        except ApplicationError:
            raise
        except (OSError, ValueError, zipfile.BadZipFile) as exc:
            if target.exists() and target.is_dir() and backup is not None and not backup.exists():
                shutil.rmtree(target, ignore_errors=True)
            if backup is not None and backup.exists() and not target.exists():
                os.replace(backup, target)
            raise ApplicationError("application_install_failed", "application package could not be installed", 500) from exc
        finally:
            if staging.exists():
    ...
    def _candidate_package_root(self) -> Path:
        root = Path(data_root()) / "application-candidates"
        root.mkdir(parents=True, exist_ok=True)
        return root

    def stage_package(self, content: bytes) -> dict[str, Any]:
        """Validate a package and put it behind the Candidate approval gate."""
        try:
            manifest, digest = validate_package(content)
        except ValueError as exc:
            raise ApplicationError(getattr(exc, "code", "invalid_package"), str(exc), 400) from exc
        package_id = uuid.uuid4().hex
        package_path = self._candidate_package_root() / f"{package_id}.zip"
        candidate = CandidateService().create({
            "kind": "application",
            "title": f"安装应用：{manifest['name']} v{manifest['version']}",
            "scope": {"kind": "application", "targets": [f"desktop/applications/{manifest['id']}"]},
            "evidence": [{"type": "package", "sha256": digest, "manifest": manifest}],
            "trace": [{"action": "package_validated", "package_id": package_id}],
            "artifact": {"package_id": package_id, "sha256": digest, "manifest": manifest},
            "patch": {"operation": "install_or_upgrade", "application_id": manifest["id"]},
            "tests": [{"name": "manifest_and_package_validation", "status": "passed"}],
            "risks": [
                {"category": "permissions", "requested": manifest.get("permissions", [])},
                {"category": "runtime", "descriptor": manifest.get("runtime")},
            ],
            "permissions": ["application.read", "application.test"],
            "rollback": {"operation": "restore_previous_application_package"},
        })
        try:
            package_path.write_bytes(content)
        except OSError as exc:
            raise ApplicationError("candidate_package_store_failed", "application package could not be staged", 500) from exc
        return {"status": "pending_approval", "candidate": candidate, "package_sha256": digest, "manifest": manifest}

    def activate_candidate(self, candidate_id: str) -> dict[str, Any]:
        candidate = CandidateService().get(candidate_id)
        if candidate.get("kind") != "application":
            raise ApplicationError("candidate_kind_mismatch", "candidate is not an application candidate", 409)
        if candidate.get("status") != "approved":
            raise ApplicationError("candidate_not_approved", "application candidate requires manual approval", 409)
        artifact = candidate.get("artifact") or {}
        package_id = artifact.get("package_id")
        expected_digest = artifact.get("sha256")
        if not isinstance(package_id, str) or not re.fullmatch(r"[a-f0-9]{32}", package_id):
            raise ApplicationError("candidate_package_missing", "candidate package reference is invalid", 409)
        package_path = (self._candidate_package_root() / f"{package_id}.zip").resolve()
        if package_path.parent != self._candidate_package_root().resolve() or not package_path.is_file():
            raise ApplicationError("candidate_package_missing", "candidate package is no longer available", 404)
        content = package_path.read_bytes()
        if not isinstance(expected_digest, str) or hashlib.sha256(content).hexdigest() != expected_digest:
            raise ApplicationError("candidate_package_changed", "staged application package hash does not match approval", 409)
        result = self.install_package(content, replace=True)
    ...
    def _plugin_invoke(self, db, row, app, manifest, input_data):
        if not isinstance(input_data, dict) or not isinstance(input_data.get("plugin_id"), str) or not isinstance(input_data.get("capability_id"), str) or not isinstance(input_data.get("input"), dict):
            raise ApplicationError("invalid_invocation", "plugin_id, capability_id and input are required")
        if not self._permission_granted(db, app.id, "plugin.invoke", manifest):
            raise ApplicationError("permission_denied", "permission required: plugin.invoke", 403)
        try:
            return PluginManager().invoke(input_data["plugin_id"], input_data["capability_id"], input_data["input"], request_id=str(input_data.get("request_id") or uuid.uuid4().hex))
        except PluginVerificationError as exc:
            raise ApplicationError("plugin_unavailable", str(exc), 503) from exc
    ...
    def invoke_capability(self, run_id, capability, method, input_data, mode="sync"):
        ...
        try:
            ...
            capability_name = "app.data" if capability.startswith("app.data") else "workspace.file" if capability.startswith("workspace.file") else "artifact" if capability.startswith("artifact") else "agent" if capability.startswith("agent") else "plugin" if capability.startswith("plugin") else capability
            required = {"workspace.file.read": "workspace.read", "workspace.file.read_external": "workspace.read", "workspace.file.list": "workspace.read", "workspace.file.write": "workspace.write", "workspace.file.delete": "workspace.write", "agent.run": "agent.run", "plugin.invoke": "plugin.invoke"}.get(f"{capability_name}.{method}")
            ...
            if capability_name == "plugin":
                if method != "invoke":
                    raise ApplicationError("capability_unavailable", "plugin method is not available", 501)
                return self._plugin_invoke(db, row, app, manifest, input_data)
