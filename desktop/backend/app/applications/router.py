...
from app.applications.manifest import ApplicationManifestError, MAX_PACKAGE_BYTES
...
async def install_application_package(package: UploadFile = File(...), replace: bool = False):
    chunks: list[bytes] = []
    total = 0
    while True:
        ...
        if total > MAX_PACKAGE_BYTES:
            return R.error("application package exceeds maximum size", code=413, data={"error_code": "package_too_large"})
        chunks.append(chunk)
    content = b"".join(chunks)
    # ``replace`` is retained for client compatibility but cannot bypass the
    # Candidate approval gate. Activation is a separate explicit operation.
    del replace
    return _call(service.stage_package, content)


@router.post("/candidates/{candidate_id}/activate")
def activate_application_candidate(candidate_id: str):
    return _call(service.activate_candidate, candidate_id)
