...
@router.get("/learning-canvases")
def list_learning_canvases(limit: int = Query(default=50, ge=1, le=100)):
    canvases = canvas_store.list_all(limit=limit)
    return R.success([canvas.model_dump(mode="json") for canvas in canvases])


@router.get("/learning-reviews/due")
def list_due_learning_reviews(limit: int = Query(default=100, ge=1, le=500)):
    """Aggregate due reviews across persisted learning spaces for APP02."""
    now = session_service._now()
    due: list[dict[str, Any]] = []
    for canvas in canvas_store.list_all(limit=100):
        ...
        for item in canvas.review_queue:
            if item.next_review_at <= now:
                ...
                due.append({
                    ...
                    "node_label": node.user_label or node.label if node else item.node_id,
                    "mastery": node.mastery if node else "unknown",
                    "next_review_at": item.next_review_at.isoformat(),
                })
    due.sort(key=lambda item: item["next_review_at"])
    return R.success(due[:limit])
