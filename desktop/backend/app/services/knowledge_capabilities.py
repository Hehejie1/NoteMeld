...
class KnowledgeCapabilityProvider:
    ...
    def manifest(self) -> list[dict[str, Any]]:
        descriptors = [
            {"id": "knowledge:article_lookup", "name": "article_lookup", "description": "按 article_id 精确读取 K0 文章事实源。", "risk": "safe", "safe": True, "parameters": {"type": "object", "properties": {"article_ids": {"type": "array", "items": {"type": "string"}}, "query": {"type": "string"}}}},
            {"id": "knowledge:evidence_search", "name": "evidence_search", "description": "在 K1 原文证据块中执行带文章和页码过滤的混合检索。", "risk": "safe", "safe": True, "parameters": {"type": "object", "required": ["query"], "properties": {"query": {"type": "string"}, "article_ids": {"type": "array", "items": {"type": "string"}}, "location": {"type": "object"}, "top_k": {"type": "integer"}}}},
            {"id": "knowledge:profile_search", "name": "profile_search", "description": "在 K2 高密文档画像中圈定候选文章。", "risk": "safe", "safe": True, "parameters": {"type": "object", "required": ["query"], "properties": {"query": {"type": "string"}, "article_ids": {"type": "array", "items": {"type": "string"}}, "filters": {"type": "object"}, "top_k": {"type": "integer"}}}},
            {"id": "knowledge:semantic_search", "name": "semantic_search", "description": "搜索 K3 实体、概念、关系及其文章证据 provenance。", "risk": "safe", "safe": True, "parameters": {"type": "object", "required": ["query"], "properties": {"query": {"type": "string"}, "article_ids": {"type": "array", "items": {"type": "string"}}, "node_types": {"type": "array"}, "relation_types": {"type": "array"}, "hops": {"type": "integer"}, "top_k": {"type": "integer"}}}},
        ]
