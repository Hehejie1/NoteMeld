...
class PluginManager:
    ...
    def invoke(self, plugin_id: str, capability_id: str, payload: dict[str, Any], *, request_id: str) -> dict[str, Any]:
        """Invoke an enabled process-jsonl plugin through the Host boundary.

        The application layer never receives the plugin path or process handle.  The
        manager resolves the active immutable version, checks the manifest and the
        granted permissions, then supervises exactly one request/response exchange.
        """
        if not isinstance(payload, dict) or not request_id.strip() or not capability_id.strip():
            raise PluginVerificationError("plugin invocation requires request_id, capability_id and object payload")
        db = SessionLocal()
        process = None
        try:
            installation = db.get(PluginInstallation, plugin_id)
            if installation is None or not installation.active_version:
                raise PluginVerificationError("plugin is not installed")
            if not installation.enabled or installation.runtime_status != "running":
                raise PluginVerificationError("plugin is not enabled")
            manifest = json.loads(installation.manifest_json)
            capability = next((item for item in manifest.get("capabilities", []) if item.get("id") == capability_id), None)
            if capability is None:
                raise PluginVerificationError("plugin capability is not declared")
            requested = {item.get("name") if isinstance(item, dict) else str(item) for item in manifest.get("requested_permissions", [])}
            granted = set(json.loads(installation.granted_permissions_json or "[]"))
            authorize_permissions({item for item in requested if item}, granted)
            runtime = manifest.get("runtime") or {}
            transport = manifest.get("transport") or {}
            if runtime.get("kind") not in {"process-jsonl", "stdin-stdout-jsonl"} and transport.get("kind") not in {"process-jsonl", "stdin-stdout-jsonl"}:
                raise PluginVerificationError("plugin transport is not process-jsonl")
            command = runtime.get("command")
            version = db.get(PluginVersion, f"{plugin_id}:{installation.active_version}")
            if version is None or not isinstance(command, list) or not command or not all(isinstance(item, str) and item for item in command):
                raise PluginVerificationError("plugin runtime is unavailable")
            root = os.path.realpath(version.path)
            relative_path = not os.path.isabs(command[0]) and ("/" in command[0] or "\\" in command[0])
            if os.path.isabs(command[0]):
                executable = os.path.realpath(command[0])
            elif relative_path:
                executable = os.path.realpath(os.path.join(root, command[0]))
            else:
                executable = os.path.realpath(shutil.which(command[0], path=os.environ.get("PATH", "")) or "")
            if not executable or not os.path.isfile(executable) or (relative_path and os.path.commonpath([root, executable]) != root):
                raise PluginVerificationError("plugin runtime executable is outside its immutable version")
            command = [executable, *command[1:]]
            request = {**payload, "operation": capability_id, "request_id": request_id}
            process = subprocess.Popen(command, cwd=root, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                       text=True, encoding="utf-8", env={"PATH": os.environ.get("PATH", ""), "NOTEMELD_PLUGIN_ID": plugin_id}, close_fds=True)
            stdout, stderr = process.communicate(json.dumps(request, ensure_ascii=False, separators=(",", ":")) + "\n", timeout=30)
            if process.returncode != 0:
                raise PluginVerificationError(f"plugin runtime exited with code {process.returncode}")
            lines = [line for line in stdout.splitlines() if line.strip()]
            if len(lines) != 1:
                raise PluginVerificationError("plugin returned an invalid JSONL response")
            try:
                result = json.loads(lines[0])
            except json.JSONDecodeError as exc:
                raise PluginVerificationError("plugin returned invalid JSON") from exc
            if not isinstance(result, dict) or result.get("request_id") not in {None, request_id}:
                raise PluginVerificationError("plugin response request_id mismatch")
            self._audit(db, plugin_id, "invoked", version=installation.active_version, detail={"capability": capability_id, "request_id": request_id, "status": result.get("status")})
            db.commit()
            return result
            ...
            if process is not None:
                process.kill()
                process.communicate()
            raise PluginVerificationError("plugin invocation timed out") from exc
        finally:
            db.close()
