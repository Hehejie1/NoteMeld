...
def list_note_library(
    *,
    ...
    """Return a bounded application-level note page from the canonical note table."""
    ...
    try:
        base = (
            ...
            .filter(NoteDocument.deleted_at.is_(None), Conversation.deleted_at.is_(None))
        )
        if normalized_query:
            base = base.filter(NoteDocument.title.ilike(f"%{normalized_query}%"))
        if wiki_status:
            base = base.filter(NoteDocument.wiki_status == wiki_status)
        if status:
            base = base.filter(NoteDocument.status == status)

        total = base.with_entities(func.count(NoteDocument.task_id)).scalar() or 0
        rows = (
            base.order_by(NoteDocument.updated_at.desc(), NoteDocument.created_at.desc())
            .offset(safe_offset)
            .limit(safe_limit)
            .all()
        )
        return {
            "items": [_document_to_dict(row, include_content=False) for row in rows],
            "pagination": {"offset": safe_offset, "limit": safe_limit, "total": int(total)},
        }
    finally:
        db.close()
