...
def test_list_learning_canvases_returns_persisted_spaces(tmp_path, monkeypatch) -> None:
    client = make_client(tmp_path, monkeypatch)
    created = client.post(
        "/api/conversations/conv_list/learning-canvases",
        json={"goal": "学习可恢复空间", "external_scopes": []},
    )
    assert created.status_code == 200

    listed = client.get("/api/learning-canvases?limit=10")

    assert listed.status_code == 200
    assert listed.json()["code"] == 0
    assert listed.json()["data"][0]["goal"] == "学习可恢复空间"


def test_due_review_endpoint_aggregates_persisted_learning_spaces(tmp_path, monkeypatch) -> None:
    client = make_client(tmp_path, monkeypatch)
    canvas = LearningCanvas(
        canvas_id="canvas_due",
        conversation_id="conv_due",
        goal="跨空间复习",
        nodes=[LearningNode(id="node_due", label="事件序号")],
        review_queue=[{"node_id": "node_due", "next_review_at": "2020-01-01T00:00:00Z"}],
    )
    learning.canvas_store.save(canvas)
    response = client.get("/api/learning-reviews/due")
    assert response.status_code == 200
    assert response.json()["data"][0]["canvas_id"] == "canvas_due"
    assert response.json()["data"][0]["node_label"] == "事件序号"
