...
def test_lan_websocket_authenticates_device_and_delivers_opaque_frame():
    ...
    service = LanDirectService(
        ...
        cipher_handshake=lambda _socket, authorization: handshake_events.append(authorization.session_id) or SessionCipher(b"k" * 32),
    )
    app = FastAPI()
    ...
    assert handshake_events == ["session-a"]
