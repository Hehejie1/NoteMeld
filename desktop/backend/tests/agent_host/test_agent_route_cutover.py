...
def test_web_tauri_and_cli_share_the_agent_v1_http_entry():
    frontend_agent = (ROOT / "desktop/frontend/src/services/agent.ts").read_text(encoding="utf-8")
    composer = (ROOT / "desktop/frontend/src/pages/HomePage/components/ChatComposer.tsx").read_text(encoding="utf-8")
    compatibility_chat = (ROOT / "desktop/frontend/src/services/chat.ts").read_text(encoding="utf-8")
    cli = (ROOT / "scripts/notemeld-agent.py").read_text(encoding="utf-8")
    desktop_entry = (ROOT / "desktop/backend/desktop_entry.py").read_text(encoding="utf-8")
    app_factory = (ROOT / "desktop/backend/app/__init__.py").read_text(encoding="utf-8")
...
def test_http_router_delegates_agent_writes_to_host_entry():
    source = (ROOT / "desktop/backend/app/routers/agent.py").read_text(encoding="utf-8")
