...
def test_installed_launcher_keeps_sdk_install_contract() -> None:
    source = (ROOT / "scripts" / "desktop" / "notemeld").read_text(encoding="utf-8")
    assert "NOTEMELD_AGENT_SDK_WHEEL" in source
...
def test_notemeld_does_not_embed_or_load_sdk_source() -> None:
    runtime = (ROOT / "desktop" / "backend" / "app" / "agent_host" / "runtime.py").read_text(encoding="utf-8")
    spec = (ROOT / "scripts" / "desktop" / "packaging" / "backend" / "pyinstaller" / "backend.spec").read_text(encoding="utf-8")
...
def test_source_and_installed_launchers_have_valid_shell_syntax() -> None:
    for script in (ROOT / "run_notemeld.sh", ROOT / "scripts" / "desktop" / "notemeld"):
...
def test_installed_launcher_skips_pip_when_sdk_versions_match() -> None:
    result = _run_ensure(ROOT / "scripts" / "desktop" / "notemeld", sdk_state="compatible", wheel=None)
    assert result.returncode == 0, result.stderr
...
def test_launcher_fails_closed_when_sdk_is_missing_or_incompatible(tmp_path: Path) -> None:
    missing = _run_ensure(ROOT / "scripts" / "desktop" / "notemeld", sdk_state="incompatible", wheel=None)
