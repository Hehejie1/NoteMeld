...
def test_document_plugin_jsonl_smoke_when_built(tmp_path, monkeypatch):
    ...
    roots = [workspace.parent / "notemeld-plugins", workspace.parent / "packages" / "notemeld-plugins"]
    binary = next(
        (
            root / "plugins" / "official-document-to-markdown" / "target" / "debug" / "notemeld-document-to-markdown"
            for root in roots
            if (root / "plugins" / "official-document-to-markdown" / "target" / "debug" / "notemeld-document-to-markdown").is_file()
        ),
        roots[0] / "plugins" / "official-document-to-markdown" / "target" / "debug" / "notemeld-document-to-markdown",
    )
