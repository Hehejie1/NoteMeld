...
def test_cloud_and_desktop_package_same_crypto_runtime() -> None:
    ...
    desktop = _requirements(ROOT / "scripts" / "desktop" / "packaging" / "backend" / "requirements-core.txt")
...
def test_backend_spec_collects_cloud_sync_modules() -> None:
    spec = (ROOT / "scripts" / "desktop" / "packaging" / "backend" / "pyinstaller" / "backend.spec").read_text(
        encoding="utf-8"
    )
...
def test_client_e2ee_is_inside_packaged_backend_namespace() -> None:
    canonical = ROOT / "desktop" / "backend" / "app" / "cloud_sync" / "e2ee.py"
