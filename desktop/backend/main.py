...
async def lifespan(app: FastAPI):
    ...
    recovered_turns = get_agent_host_entry().recover_nonterminal()
    ...
    try:
        ...
        descriptor = write_descriptor(
            ...
            AgentRuntimeDescriptor(
                ...
                sdk_version=str(host._loaded.sdk_version),
                abi_version=int(host._loaded.abi_version),
        ...
        # The Cloud Host can be installed after startup by the authenticated
        # desktop bootstrap endpoint. Read app.state again so that runtime is
        # not leaked when the process exits.
        active_cloud_sync_runtime = getattr(getattr(app, "state", None), "cloud_sync_runtime", None)
        if active_cloud_sync_runtime is not None:
            active_cloud_sync_runtime.close()
