...
const buildScript = await readFile(path.join(root, 'packaging/scripts/build-desktop-macos.sh'), 'utf8')
const buildBackendScript = await readFile(path.join(root, 'packaging/scripts/build-backend-macos.sh'), 'utf8')
const skillScript = await readFile(
  path.join(root, '.trae/skills/notemeld-dmg-packaging/scripts/package-notemeld-dmg.sh'),
  'utf8',
)
