...
assert.match(
  appLayoutSource,
  /mt-6 flex min-h-0 flex-1 flex-col overflow-hidden/,
...
assert.match(
  appLayoutSource,
  /relative flex h-8 min-w-0 w-full max-w-full items-center gap-2 overflow-hidden/,
  '左侧笔记列表按钮必须限制 flex 宽度，让标题可以按剩余空间省略',
)

assert.match(
  appLayoutSource,
  /block w-0 min-w-0 flex-1 truncate/,
