...
test('device list exposes online status, LAN candidates, and revoke action', () => {
  assert.match(devices, /device\.online/); assert.match(devices, /lan_endpoints/); assert.match(devices, /revokeDevice/); assert.match(devices, /aria-label="Connected devices"/)
})
