...
export class CloudClient {
  ...
  sharedSnapshot(sessionId: string, shareToken: string) { return this.request<Record<string, unknown>>('GET', `/v1/shared/${encodeURIComponent(sessionId)}/snapshot`, undefined, { headers: { 'X-Share-Token': shareToken } }) }
  sharedEvents(sessionId: string, shareToken: string, after = 0) { return this.request<unknown[]>('GET', `/v1/shared/${encodeURIComponent(sessionId)}/events`, undefined, { params: { after }, headers: { 'X-Share-Token': shareToken } }) }
