...
std::vector<Bridge *> deferred_releases;
...
void DrainDeferred(JNIEnv *env) {
    if (!env) return;
    std::vector<Bridge *> pending;
    { std::lock_guard<std::mutex> guard(bridge_mutex); pending.swap(deferred_releases); }
    for (auto *bridge : pending) {
        env->DeleteGlobalRef(bridge->runtime);
        delete bridge;
    }
}
...
void ReleaseBridge(void *context) {
    ...
    if (!env) {
        ...
        deferred_releases.push_back(bridge);
        return;
    }
    DrainDeferred(env);
    env->DeleteGlobalRef(bridge->runtime);
...
extern "C" void JNI_OnUnload(JavaVM *java_vm, void *) {
    JNIEnv *env = nullptr;
    if (java_vm->GetEnv(reinterpret_cast<void **>(&env), JNI_VERSION_1_6) == JNI_OK) DrainDeferred(env);
}
#define JNI(name) Java_wiki_notemeld_agent_Runtime_##name
extern "C" JNIEXPORT jstring JNICALL JNI(nativeSdkVersion)(JNIEnv *e,jclass){DrainDeferred(e);return NewJavaString(e,notemeld_agent_sdk_version());}
extern "C" JNIEXPORT jstring JNICALL JNI(nativeSchemaVersion)(JNIEnv *e,jclass){DrainDeferred(e);return NewJavaString(e,notemeld_agent_schema_version());}
extern "C" JNIEXPORT jlong JNICALL JNI(nativeNew)(JNIEnv *e,jclass,jstring v){DrainDeferred(e);std::string s;if(!JavaString(e,v,&s))return 0;return reinterpret_cast<jlong>(notemeld_agent_runtime_new(s.c_str()));}
extern "C" JNIEXPORT jint JNICALL JNI(nativeSetCallbacks)(JNIEnv *e,jclass,jlong raw,jobject runtime){DrainDeferred(e);auto *h=reinterpret_cast<AgentRuntimeHandle *>(raw);if(!h||!runtime)return -2;auto global=e->NewGlobalRef(runtime);if(!global)return -9;auto *b=new Bridge{h,global};auto code=notemeld_agent_runtime_set_callbacks(h,EventCallback,b,DriverCallback,b,ReleaseBridge,b);if(code==0){std::lock_guard<std::mutex> g(bridge_mutex);bridges[h]=b;}else{e->DeleteGlobalRef(global);delete b;}return code;}
extern "C" JNIEXPORT jlong JNICALL JNI(nativeSubmit)(JNIEnv *e,jclass,jlong raw,jstring v){DrainDeferred(e);std::string s;if(!JavaString(e,v,&s))return 0;return notemeld_agent_submit_turn(reinterpret_cast<AgentRuntimeHandle *>(raw),s.c_str());}
extern "C" JNIEXPORT jint JNICALL JNI(nativeCancel)(JNIEnv *e,jclass,jlong raw,jlong token){DrainDeferred(e);return notemeld_agent_cancel_turn(reinterpret_cast<AgentRuntimeHandle *>(raw),token);}
extern "C" JNIEXPORT jint JNICALL JNI(nativeWait)(JNIEnv *e,jclass,jlong raw,jlong token,jlong timeout){DrainDeferred(e);return notemeld_agent_wait_turn(reinterpret_cast<AgentRuntimeHandle *>(raw),token,timeout);}
extern "C" JNIEXPORT jint JNICALL JNI(nativeSteer)(JNIEnv *e,jclass,jlong raw,jlong token,jstring v){DrainDeferred(e);std::string s;if(!JavaString(e,v,&s))return -2;return notemeld_agent_steer_turn(reinterpret_cast<AgentRuntimeHandle *>(raw),token,s.c_str());}
extern "C" JNIEXPORT void JNICALL JNI(nativeFree)(JNIEnv *e,jclass,jlong raw){DrainDeferred(e);notemeld_agent_runtime_free(reinterpret_cast<AgentRuntimeHandle *>(raw));}
