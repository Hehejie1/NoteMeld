"""Small, dependency-free interactive client for evaluating the SDK with Ollama."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Evaluate NoteMeld Agent SDK with a local Ollama model")
    parser.add_argument("--model", default=os.environ.get("NOTEMELD_OLLAMA_MODEL", "qwen3:4b"))
    parser.add_argument("--ollama-url", default=os.environ.get("OLLAMA_URL", "http://127.0.0.1:11434"))
    parser.add_argument("--library", default=os.environ.get("NOTEMELD_AGENT_SDK_LIBRARY"))
    args = parser.parse_args(argv)
    history: list[dict[str, Any]] = []
    model = args.model
    def driver(request: dict[str, Any]) -> dict[str, Any]:
        return _ollama(args.ollama_url, model, request)
    try:
        with Runtime(args.library, driver=driver) as runtime:
            ...
            while True:
                ...
                token = runtime.submit_turn(request)
                runtime.wait(token, 300_000)
                while True:
