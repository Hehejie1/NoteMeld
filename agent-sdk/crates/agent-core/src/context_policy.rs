...
impl ContextPolicy {
 ...
 pub fn trim_messages(&self,messages:&[AgentMessage])->(Vec<AgentMessage>,Option<ContextTrimDiagnostic>) { let available=self.budget().available_tokens; let mut kept=messages.to_vec(); let mut estimate=estimate(&kept); let original=kept.len(); while estimate>available&&kept.len()>2 { kept.remove(1); estimate=estimate(&kept); } let diag=(original!=kept.len()).then_some(ContextTrimDiagnostic{removed_messages:original-kept.len(),estimated_input_tokens:estimate}); (kept,diag) }
}
