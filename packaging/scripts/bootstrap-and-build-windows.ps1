...
# 用法 (PowerShell):
#   cd <repo_root>
#   .\packaging\scripts\bootstrap-and-build-windows.ps1
#
# 可选参数:
...
$BuildScript   = Join-Path $RootDir "packaging\scripts\build-desktop-windows.ps1"
...
排查思路 (按出现顺序):
  1) PyInstaller 失败 -> 看错误中的 'ModuleNotFoundError', 在
     packaging\backend\pyinstaller\backend.spec 的 hiddenimports 里补包名
