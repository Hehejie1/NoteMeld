...
function Ensure-Installed {
  ...
  if (-not (Test-Path (Join-Path $AppDir "backend"))) { Fail "Backend directory not found at $AppDir\backend." }
  if (-not (Test-Path (Join-Path $AppDir "frontend"))) { Fail "Frontend directory not found at $AppDir\frontend." }
...
function Cmd-Doctor {
  ...
  if (-not (Test-Path (Join-Path $AppDir "frontend\node_modules"))) { Fail "Missing frontend dependencies: $AppDir\frontend\node_modules" }
...
function Cmd-Update {
  ...
  try {
    ...
    Invoke-CheckedCommand (Join-Path $AppDir ".venv\Scripts\python.exe") @("-m", "pip", "install", "-r", "backend\requirements.txt") "Failed to install backend requirements"
    Repair-FrontendDependencies (Join-Path $AppDir "frontend")
  } finally {
    Pop-Location
  }
