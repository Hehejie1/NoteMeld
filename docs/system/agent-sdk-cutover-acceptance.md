...
| 19 | 部分通过 | backend Agent/Host 相关契约 114 passed（另 4 xfailed）；Rust/clippy/frontend/CLI/native cancel/restart recovery 通过；Tauri 单测 7 passed，窗口退出不再终止共享 Host；packaged sidecar 启动 smoke 仍需修复启动阶段无日志超时 |
