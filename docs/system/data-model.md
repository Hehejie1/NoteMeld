...
Cloud v1 stores session metadata/events, command idempotency records, and hashed scoped bearer/share-token metadata in `cloud.db`; bearer token rows also persist audience and scope JSON, while raw login, PAT and share-token values are returned only at issuance and are never persisted. Workspace content is under the configured `workspaces/<user_id>/<workspace_id>` directory, with ZIP backups under `backups/<user_id>/<workspace_id>`. Workspace writes use a fsync + atomic replace sequence and reject traversal/symlink access. Restore validates archive paths and size before atomically overlaying files. Relay payloads are validated as `notemeld.sync.v1` envelopes, routed to the addressed connected device, and are not written to SQLite or disk.
