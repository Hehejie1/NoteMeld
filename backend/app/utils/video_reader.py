...
class VideoReader:
    def __init__(self,
        ...
        self.frame_dir = frame_dir or self._build_default_dir(get_app_dir("output_frames"), video_path)
        self.grid_dir = grid_dir or self._build_default_dir(get_app_dir("grid_output"), video_path)
        self.font_path = font_path

    @staticmethod
    def _build_default_dir(base_dir: str, video_path: str) -> str:
        stem = re.sub(r"[^A-Za-z0-9._-]+", "_", os.path.splitext(os.path.basename(video_path))[0]).strip("._-")
        digest = hashlib.sha256(os.path.abspath(video_path).encode("utf-8")).hexdigest()[:12]
        return os.path.join(base_dir, f"{stem or 'video'}_{digest}")

    @staticmethod
    def _clear_directory(directory: str, prefixes: tuple[str, ...]) -> None:
        os.makedirs(directory, exist_ok=True)
        for file in os.listdir(directory):
            file_path = os.path.join(directory, file)
            if os.path.isfile(file_path) and file.startswith(prefixes):
                os.remove(file_path)
    ...
    def extract_frames(self, max_frames=1000) -> list[str]:
        ...
        try:
            ...
            timestamps = self._build_timestamps(duration, max_frames)
            if not timestamps:
                return []
    ...
    def _build_timestamps(self, duration: float, max_frames: int) -> list[int]:
        if self.frame_timestamps:
            normalized = {
                max(0, int(round(float(timestamp))))
                for timestamp in self.frame_timestamps
                if timestamp is not None
            }
            return sorted(timestamp for timestamp in normalized if duration <= 0 or timestamp < duration)[:max_frames]
        return [i for i in range(0, int(duration), self.frame_interval)][:max_frames]
    ...
    def run(self)->list[str]:
        logger.info("开始提取视频帧...")
        try:
            self._clear_directory(self.frame_dir, ("frame_",))
            self._clear_directory(self.grid_dir, ("grid_",))
            self.extract_frames()
            logger.info("开始拼接网格图...")
            image_paths = []
