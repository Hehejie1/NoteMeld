...
class LocalDownloader(Downloader, ABC):
    ...
    def download_video(self, video_url: str, output_dir: str = None) -> str:
        ...
        if video_url.startswith('/uploads'):
            video_url = os.path.join(str(upload_dir()), video_url.removeprefix('/uploads/'))
    ...
    @staticmethod
    def _is_audio_file(input_path: str) -> bool:
        return os.path.splitext(input_path)[1].lower() in AUDIO_EXTENSIONS
    ...
    def download(
        ...
        if self._is_audio_file(video_url):
            if video_url.lower().endswith('.mp3'):
                file_path = video_url
            else:
