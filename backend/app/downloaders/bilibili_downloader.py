...
class BilibiliDownloader(Downloader, ABC):
    def __init__(self):
        ...
        self._refresh_cookie_context()

    def _refresh_cookie_context(self) -> None:
        latest_cookie = self._cookie_mgr.get('bilibili')
        if latest_cookie == self._cookie and self._cookiefile:
            return
        self._cleanup_cookiefile()
        self._cookie = latest_cookie
        self._cookiefile = self._write_netscape_cookie_file()

    def _cleanup_cookiefile(self) -> None:
        if self._cookiefile and os.path.exists(self._cookiefile):
            try:
                os.remove(self._cookiefile)
            except OSError:
                logger.warning("清理 B站 Cookie 临时文件失败: %s", self._cookiefile)
        self._cookiefile = None
    ...
    def download(
        ...
        try:
            ...
            if "HTTP Error 412" in str(e):
                if not self._cookie:
                    raise RuntimeError(
                        "B站返回 412，当前后端没有可用的 B站 Cookie。"
                        "请在设置页配置有效的 SESSDATA Cookie 后重试。"
                    ) from e
                raise RuntimeError(
                    "B站返回 412，当前 Cookie 可能已失效。"
                    "请重新从浏览器复制最新的 B站 Cookie（至少包含 SESSDATA）后重试。"
                ) from e
            raise
